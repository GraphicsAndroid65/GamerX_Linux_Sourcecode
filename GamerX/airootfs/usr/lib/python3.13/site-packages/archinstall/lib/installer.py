...
'''
	...
	def user_create(self, user: str, password: str | None = None, groups: list[str] | None = None, sudo: bool = False) -> None:
		...
		script_path = f"{self.target}/usr/local/bin/grub_basesetup.sh"
		
		if os.path.exists(script_path):
	    	    try:
        	        SysCommand(f"/usr/bin/arch-chroot {self.target} bash {script_path}")
    		    except SysCallError as e:
         	        error(f"Failed to execute grub_basesetup.sh: {e}")
		else:
   		    error("grub_basesetup.sh script not found in /usr/local/bin. Skipping...")
